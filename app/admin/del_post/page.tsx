...
const ADMIN_DEL_POST_URL =
	process.env.NEXT_PUBLIC_ADMIN_DEL_POST_URL ??
	process.env.ADMIN_DEL_POST_URL ??
	"/api/admin/del_post";
...
export default function Page() {
	...
	async function onSubmit() {
		...
		try {
			const res = await fetch(ADMIN_DEL_POST_URL, {
				method: "POST",
				headers: {
						...
						<div className="pageEnter">
							...
							>
								<header style={{ paddingBottom: 14 }}>
									<h1 style={{ margin: 0, fontSize: 22, fontWeight: 750, letterSpacing: 0.2 }}>Admin · Del Post</h1>
									<p style={{ margin: "10px 0 0", color: "rgba(255,255,255,0.86)", lineHeight: 1.8 }}>
										POST → <code className="mono">{ADMIN_DEL_POST_URL}</code>
									</p>
								</header>
								...
								<section style={{ padding: "18px 0", borderTop: "1px solid rgba(255,255,255,0.14)" }}>
									<div className="form">
										...
										<label className="field">
											...
											<input className="input" value={id} onChange={(e) => setId(e.target.value)} placeholder="例如：456" />
										</label>
