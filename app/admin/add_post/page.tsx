...
const ADMIN_ADD_POST_URL =
	process.env.NEXT_PUBLIC_ADMIN_ADD_POST_URL ??
	process.env.ADMIN_ADD_POST_URL ??
	"/api/admin/add_post";

// 轻量 Markdown 预览：不引依赖（标题/粗体/斜体/行内代码/链接/段落/代码块）
...
export default function Page() {
	...
	const [tags, setTags] = useState(""); // 可选：逗号分隔（按后端约定)

	const [submitting, setSubmitting] = useState(false);
	const [result, setResult] = useState<string>("");

	const canSubmit = useMemo(() => token.trim() && title.trim(), [token, title]); // was token/title/date
	...
	async function onSubmit() {
		...
		try {
			const res = await fetch(ADMIN_ADD_POST_URL, {
				method: "POST",
				headers: {
						...
						<div className="pageEnter">
							...
							>
								<header style={{ paddingBottom: 14 }}>
									<h1 style={{ margin: 0, fontSize: 22, fontWeight: 750, letterSpacing: 0.2 }}>Admin · Add Post</h1>
									<p style={{ margin: "10px 0 0", color: "rgba(255,255,255,0.86)", lineHeight: 1.8 }}>
										POST → <code className="mono">{ADMIN_ADD_POST_URL}</code>
									</p>
								</header>
								...
								<section style={{ padding: "18px 0", borderTop: "1px solid rgba(255,255,255,0.14)" }}>
									<div className="grid">
										<div className="form">
											...
											<label className="field">
												<span className="label">tags（可选）</span>
										...
										<aside className="previewPane" aria-label="实时预览">
											...
											<div className="previewBody">
												{tags.trim() ? <p className="previewDesc">tags: {tags}</p> : null}
