...
// 后端接口：GET /daily?id=1 → DailyItem
const ENTRY_API_URL =
	process.env.DIARY_ENTRY_URL ?? "http://127.0.0.1:8000/daily";
...
async function fetchEntry(params: {
	...
	const url = `${ENTRY_API_URL}?id=${encodeURIComponent(params.id)}`;
	...
	try {
		...
		return {
			...
			date: data.date != null ? String(data.date)
				: data.time != null ? new Date(Number(data.time) * 1000).toISOString().slice(0, 10)
				: undefined,
...
export default async function Page({
	...
	// FIX: Next 16 / Turbopack 下 searchParams 可能是 Promise
	searchParams?: SearchParams | Promise<SearchParams>;
}) {
	const sp = await Promise.resolve(searchParams ?? ({} as SearchParams));

	const spTitle = firstParam(sp.title);
	const spDate = firstParam(sp.date);
	const spDesc = firstParam(sp.desc);
	const spLink = firstParam(sp.link);
	const spId = firstParam(sp.id);

	const fallbackTitle = spTitle ?? "未命名";
	const fallbackDate = spDate ?? "";
	const fallbackDesc = spDesc ?? "";
	// const fallbackLink = spLink ?? ""; // 删除：不再需要
	...
	// const link = detail?.link ?? fallbackLink; // 删除：按钮已移除

	// 正文：优先接口返回的 markdown，否则用 desc 做兜底
	const markdown = (detail?.content ?? detail?.md ?? fallbackDesc).trim();
			...
			>
				<div style={{ maxWidth: 920, margin: "0 auto", padding: "0 18px 54px" }}>
					...
					>
						<div className="pageEnter">
							...
							>
								...
								{/* 右下角：返回列表 */}
								<Link href="/diary" className="backHome" aria-label="返回游记/杂谈列表">
					...
					<style>{`
						...
						/* Markdown 排版（尽量贴合现有风格） */
						.md{
							color: rgba(255,255,255,0.86);
							line-height: 1.9;
							font-size: 14px;
							--md-p-size: 15px; /* NEW: 统一控制普通段落字号 */
							max-width: 100%;
						...
						.md :where(p){
							margin: 0 0 12px;
							font-size: var(--md-p-size); /* was 15px */
						}
